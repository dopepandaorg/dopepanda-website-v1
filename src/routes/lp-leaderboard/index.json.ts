...
const LP_VALIDATE_API = (time: string, endTime: string) =>
	`https://algoindexer.algoexplorerapi.io/v2/accounts/PMSLU3PDSQ4RTD7PB3MYXWNQL6INRLBYMNX7JTUWT2QXFKAI66DQQDRNVQ/transactions?tx-type=axfer&after-time=${time}&before-time=${endTime}&asset-id=552661375&limit=5000`;
...
export const get: RequestHandler = async ({}) => {
	...
	const lpValidateAPIs = await fetch(
		LP_VALIDATE_API(
			new Date(latestSnapshotDate).toISOString(),
			new Date(nextSnapshotDate).toISOString()
		)
	);
	...
	let invalidAccounts = [];
	let newAccounts = [];
	let accounts = lpHoldersResponse.accounts.filter(
		(a) => ignoredAccounts.indexOf(a.address) === -1
	);
	...
	accounts = accounts.map((a) => {
		...
		const isValid =
			snapshotLp > lpCutoff && balance >= snapshotLp && typeof isUnder[a.address] === 'undefined';
		let status =
			isValid && typeof isUnder[a.address] === 'undefined'
		...
		if (snapshotLp === 0 && balance > lpCutoff)
			status = { type: 'INVALID_NEW', label: 'New', data: null };
		else if (balance < lpCutoff && balance > 0)
			status = { type: 'INVALID', label: 'Not Eligible', data: null };
