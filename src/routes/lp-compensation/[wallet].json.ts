...
const COMPENSATION_CHECK = (price: number, asset2: number, asset1: number) =>
	price < LP_PRICE ? (asset2 - asset1 / LP_PRICE) / 2 : 0;
	...
	if (walletResponse.transactions) {
		walletResponse.transactions.map((t: any) => {
			if (
				...
				t['asset-transfer-transaction']['asset-id'] === 391379500
			) {
			...
			if (
				...
				t['payment-transaction'].receiver === wallet
			) {
			...
			if (
				...
				t['tx-type'] === 'axfer' &&
				t['asset-transfer-transaction'] &&
				t['asset-transfer-transaction'].receiver === wallet
			) {
				foundTxsTotal -= t['asset-transfer-transaction'].amount / MULTIPLIER;
			}
		});
